...
def __validate(file_path, type):
    """
    Start the validation of files
    """
    h = {'Content-Type': "{}; charset=utf-8".format(type)}
    # This would open files in binary mode:
    ...
    for m in messages:
        # Capture files that have incomplete / broken HTML
...
def __analyse(file_path):
    """Start analyse of file and print the result
    """
    nb_errors = 0
    try:
        result = None
...
if __name__ == "__main__":
    ...
    """execute tests, exit. Exit status = # of errors (0 on success)
    """
